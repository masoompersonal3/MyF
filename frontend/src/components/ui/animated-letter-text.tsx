...
export function AnimatedLetterText({ text = "Portfolio", letterToReplace = "o", className }: AnimatedLetterTextProps) {
  ...
  if (replaceIndex === -1) {
    return <span className={`font-black tracking-tight text-white ${className ?? ""}`}>{text}</span>;
  }
  ...
  return (
    <span className={`inline-flex items-center font-black tracking-tight text-white ${className ?? ""}`}>
      ...
      >
        <svg className="absolute w-0 h-0" aria-hidden="true">
          <defs>
            <filter id="innerShadow" x="-50%" y="-50%" width="200%" height="200%">
              ...
              <feGaussianBlur stdDeviation="3" /><feOffset dx="0" dy="2" result="offsetblur" />
              <feFlood floodColor="rgba(255,255,255,0.15)" result="color" />
              <feComposite in2="offsetblur" operator="in" /><feComposite in2="SourceAlpha" operator="in" />
        ...
        <svg viewBox="0 0 100 100" className="w-[0.75em] h-[0.75em]">
          <path d="M50 0 C55 15,65 15,75 10 C70 25,75 35,90 35 C80 45,80 55,90 65 C75 65,70 75,75 90 C65 85,55 85,50 100 C45 85,35 85,25 90 C30 75,25 65,10 65 C20 55,20 45,10 35 C25 35,30 25,25 10 C35 15,45 15,50 0Z" fill="url(#outerShapeGradient)" filter="url(#innerShadow)" />
          <path d="M50 0 C55 15,65 15,75 10 C70 25,75 35,90 35 C80 45,80 55,90 65 C75 65,70 75,75 90 C65 85,55 85,50 100 C45 85,35 85,25 90 C30 75,25 65,10 65 C20 55,20 45,10 35 C25 35,30 25,25 10 C35 15,45 15,50 0Z" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="1" />
        </svg>
        <span className="absolute inset-0 flex items-center justify-center">
          <svg viewBox="0 0 100 100" className="w-[0.32em] h-[0.32em] animate-[spin_3s_linear_infinite]" style={{ filter: "url(#diamondGlow)" }}>
